...
program
  ...
  .alias("gen")
  .description(
    "Generate a LQ file (if a file was passed in or multiple files if a folder was given)."
  )
  .description(
    "By default generates files in the same folder as the given folder. Can generate into a custom folder, if you pass it as the second parameter."
  )
      ...
      fs.readdir(folder, function (err, items) {
        for (var i = 0; i < items.length; ++i) {
          try {
            ...
            if (!itemCheck.isFile() || /-lq\..+$/.test(file)) continue;
            generator
